...
import {
	...
	GetEmployeeTasksUseCase,
} from "@useCases/employees/getEmployeeTasksUseCase";
...
const getEmployeeTasksControllerSchema = z.object({
...
type GetEmployeeTasksRequest = z.infer<typeof getEmployeeTasksControllerSchema>;

export class GetEmployeeTasksController {
	constructor(private getEmployeeTasksUseCase: GetEmployeeTasksUseCase) {}

	async handle(request: Request, response: Response): Promise<Response> {
		const employeeId = request.params.id;
		const { role, department, id, companyId } = request.employee;
		...
		for (const [key, value] of Object.entries(queries)) {
			...
			if (Array.isArray(value)) {
				parsedQuery[section][field] = [];
				for (const val of value) {
					if (!parsedQuery[section][field].includes(val)) {
						parsedQuery[section][field].push(val);
					}
				}
			} else {
		...
		const parsedParams: GetEmployeeTasksRequest = await validateParams.parse(
			getEmployeeTasksControllerSchema,
			{
				employeeId,
				requester: {
					companyId,
		...
		const requestResult = await this.getEmployeeTasksUseCase.execute(parsedParams);

		return response.json(requestResult);
	}
}

export const getEmployeeTasksController = new GetEmployeeTasksController(getEmployeeTasksUseCase);
